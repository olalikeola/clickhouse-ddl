...
describe('ClickHouse DDL Parser - Comprehensive Tests', () => {
  ...
  describe('CREATE VIEW - Phase 1', () => {
    it('parses simple CREATE VIEW', () => {
      ...
      // AST assertions
      expect(result.view?.select).toBeDefined()
      expect(result.view?.select.type).toBe('SELECT')
      expect(result.view?.select.columns).toHaveLength(2)

      // First column: id
      const col1 = result.view?.select.columns[0].expression
      expect(col1?.type).toBe('COLUMN')
      if (col1 && col1.type === 'COLUMN') {
        expect(col1.name).toBe('id')
      }

      // Second column: name
      const col2 = result.view?.select.columns[1].expression
      expect(col2?.type).toBe('COLUMN')
      if (col2 && col2.type === 'COLUMN') {
        expect(col2.name).toBe('name')
      }
